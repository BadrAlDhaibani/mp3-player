...
class ItemColumn(QWidget):
    ...
    def _paint_item(self, painter: QPainter, index: int, item: Item, y: int) -> None:
        ...
        if active:
            ...
            # slab behind "Rescan" reads as a banner rather than a cursor.
            # Batch 5 turns this into a real glow; for now it's what says where
            # you are. Rows with a readout run the full width so the values stay
            # in a column; rows without one shrink to fit their label.
            plate = (
                available
                if item.value
